#!/bin/bash
# Diagnostic: does the Remove-OT (std CFM, no OT coupling) condition also
# collapse relative to baseline under the fixed code, or does it do better
# than OT-CFM (pointing at OT coupling specifically as the problem) or
# equally bad (pointing at the checkpoint-training approach more broadly)?
# Same 3 objects/seeds as the DDPM check for a direct comparison.
set -u
...
CKPT="grasp_6dof/models/cfm_allobj.pt"
OUT_DIR="/lena/projects/OWG-main/paperA_data/phase0_diag_extended"
mkdir -p "$OUT_DIR"

for obj in "${OBJECTS[@]}"; do
  raw="$OUT_DIR/removeOT_check_${obj}.jsonl"
  > "$raw"
  succ=0
  for seed in "${SEEDS[@]}"; do
    ...
    echo "{\"condition\":\"removeOT\",\"object\":\"$obj\",\"seed\":$seed,\"success\":\"$SUCCESS\"}" >> "$raw"
  done
  echo "[removeOT-check-$obj] === DONE: $succ/${#SEEDS[@]} ==="
done
