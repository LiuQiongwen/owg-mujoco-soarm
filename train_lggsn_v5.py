#!/usr/bin/env python3
"""
LGGSN v5 — 17-dim geom + 4-dim query embedding (nn.Embedding over 7 classes).

Changes from v3:
  - Trained on 5 objects only: banana, can, pear, mustard, drill
    (CrackerBox and Scissors excluded — execution-layer failures)
  - Each episode carries a query_id (class integer) fed to LGGSN query_emb
  - LGGSN: n_queries=7, geom_dim=17, query_dim=4 → MLP input = 21
  - Everything else identical to v3 (cross-episode BPR, 80/20 split)
...
CKPT_PATH  = os.environ.get(
    "LGGSN_CKPT",  "grasp_6dof/models/lggsn_pairwise_live_v5.pt"
)
...
def load_episodes(path):
    """
    Read v3 JSONL, filter to TRAIN_OBJECTS, group by (query, scene_id).
...
def main():
    ...
    print(f"\nDevice     : {device}")
    print(f"Model      : LGGSN  n_queries=7  geom_dim=17  query_dim=4  "
          f"mlp_in=21  hidden=40  dropout={DROPOUT}  params={n_params}")
    ...
    print(f"Saved v5 checkpoint → {CKPT_PATH}")
