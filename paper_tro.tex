...
tabular ML code via static analysis; closer in mechanism, an
abstract-interpretation-based static analysis \cite{urbanleakage2022}
proves the absence of leakage in Jupyter notebooks at development time.
...
\subsection{Boundary checks against three real public codebases}
\label{sec:externaltagger}

\Cref{sec:external} manually checked the criterion against two published
systems' stated designs; this section instead runs the automated tagger
itself (\cref{sec:autotag}) against real, unmodified external source code,
to check whether it applies outside this project's own pipelines at all.
We deliberately do not call this a validation of the tagger's sensitivity:
none of the three codebases below yielded a confirmed real-world
violation, so what follows is a set of boundary/specificity checks and two
genuine tool-development findings, not a demonstration that the tagger
catches real external defects -- that would require an external, labeled
true-positive corpus this investigation does not have.

Two of three checked codebases are causally clean \emph{by construction},
not by any property the tagger detected: GraspGen \cite{graspgen2025}
(re-examined here at the code level, corroborating \cref{sec:external}'s
citation-level claim) and Dex-Net's grasp-quality scoring
\cite{mahler2017dexnet} both compute candidate scores purely analytically
from geometry -- wrench-space/friction-cone metrics for Dex-Net, a
point-cloud-and-pose discriminator for GraspGen -- with no
execution-contingent input anywhere in the released code. Neither has an
execution boundary for our criterion to be tested against at all, so a
clean result here says nothing about detector sensitivity, only that
nothing was misclassified as pre-execution when it should not have been.

Sim-Grasp \cite{ligrasp2024simgrasp} does have a genuine
candidate-pool$\to$physical-simulation$\to$label loop
(\texttt{main\_simulation\_loop}), and running the tagger against it
produced two real findings. First, it confirms one of
\cref{sec:autotag}'s own documented limitations is not merely a
synthetic-fixture artifact: the loop drives execution via
\texttt{world.step(...)} rather than \texttt{env.step(...)}, and the
tagger's hardcoded \texttt{env}-named-handle check misses it on real,
unmodified external code exactly as predicted. Second, and more valuable
methodologically, tracing the call graph before placing any marker caught
a near-miss: the field this loop writes, \texttt{simulation\_quality},
initially looked like a candidate-scoring feature worth flagging, but the
actual grasp action is issued by a separate, earlier function --
\texttt{main\_simulation\_loop} only records a post-hoc stress-test
outcome as a label, and labels are explicitly exempt from the
pre-execution requirement by this project's own corollary
(\cref{sec:method}). Reporting this as a violation would have been a
false, unsupportable claim, caught by exactly the manual-verification
discipline this project's own self-corrections (\cref{sec:selfcorrections})
already established as necessary -- the audit \emph{process}, not just
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_critic_results.pdf}
\caption{\Cref{tab:critic_results} as a bar chart. The stale checkpoint's success rate is not shown here since it comes from a disjoint scene population (\texttt{phase1/} base-42, not paired with either batch above) --- see \cref{tab:critic_results}'s caption for its AUROC under the same corrected harness.}
\label{fig:critic_results}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_offline_vs_live_consistency.pdf}
\caption{Live-executed (\cref{sec:critic}) and offline re-scored (\cref{sec:multihead}) results on the same confirmatory scene population, shown side by side. These are two different, complementary measurements and are never pooled or averaged into a single number --- the figure exists to show they agree in direction and rough magnitude, not to combine them.}
\label{fig:offline_vs_live}
\end{figure}
...
\bibitem{urbanleakage2022}
F.~Drobnjaković, P.~Subotić, and C.~Urban,
``Abstract Interpretation-Based Data Leakage Static Analysis,''
\emph{arXiv:2211.16073}, 2022.
