...
verification. These are boundary/specificity checks against three
systems, not a benchmark: we make no claim that any codebase is
exhaustively verified clean, only what is reported for the specific
paths inspected.

Two of the three are causally clean \emph{by construction}, not by any
property the tagger detected: GraspGen (already checked at the code
level in \cref{sec:external}) and Dex-Net's grasp-quality scoring
\cite{mahler2017dexnet} both compute
candidate scores purely analytically from geometry -- a point-cloud-and-
pose discriminator for GraspGen, wrench-space/friction-cone metrics for
Dex-Net -- with no execution-contingent input anywhere in the released
scoring code. Neither has an execution boundary for our criterion to be
tested against at all, so a clean result here says nothing about
detector sensitivity, only that nothing was misclassified as
pre-execution when it should not have been.
...
predating \cref{sec:method} entirely). Reproducing a predecessor
checkpoint's previously reported $+15.6$-point gain directly against the
original data, we found it invalid on two grounds unrelated to the
criterion itself -- a seed-coupling bug that defeated its paired
evaluation design, and a success criterion that saturated regardless of
candidate quality -- and, once both were fixed, that checkpoint fails
outright under the corrected harness: AUROC $=0.4996$ on 1{,}500 real
per-candidate labels, chance level. We rebuilt the critic
causally-admissible by construction (candidate pose relative to the
target object, point-cloud statistics, object identity, every field
registered \texttt{PRE\_EXECUTION} and checked by the same gate as this
paper's other pipelines): live-executed on two scene batches disjoint
from training and from each other, it beats a geometric baseline on
both, $+15.6$ and $+14.0$ percentage points (exact McNemar $p<0.003$
both). The full architecture, training procedure, and this
investigation's own honestly-reported negative and incomplete findings
(an uncertainty risk gate with no measurable benefit; the pairwise loss
term's independent contribution left unresolved at the current data
scale; a small-data imitation-policy pilot that fails its first rollout)
are reported in full in a companion submission dedicated to this critic
-- consistent with this paper's own house style of not re-presenting a
result as a second paper's novelty, the same convention that companion
paper applies to this one. We summarize here only what is needed to
illustrate the criterion applied prospectively, not retrospectively, and
to support \cref{sec:fourobj}'s extension of this same critic to a
fourth object.

\subsection{Mechanistic analysis: multi-head decomposition (companion result)}
\label{sec:multihead}

A second, complementary experiment on the same three-object critic
(\cref{sec:critic}) -- explicitly offline re-scoring, never pooled with
\cref{sec:critic}'s live-executed numbers -- decomposes prediction into
four heads (bilateral contact, lift, success, and a three-class
failure-type) to ask \emph{why and where} the critic works, not just
whether it does. On the frozen confirmatory batch, the success head
alone reaches $+10.7$ points over geometry (exact McNemar $p=0.0052$),
directionally consistent with \cref{sec:critic}'s live-executed result,
with a specific, actionable asymmetry a single-head critic cannot show:
the model is conservative rather than overconfident on success. Full
derivation, all four result tables, and the load-bearing per-object
limitation this decomposition surfaces (a failure-type class with
support collapsing to a single example once one object is excluded) are
reported in the companion submission; \cref{sec:fourobj} extends this
same offline-decomposition methodology to a fourth object.
...
a learned robustness signal and an uncertainty-penalized heuristic both
