...
%% confirmed from this environment; retried 2026-08-05, same 403, consistent not transient). Also
%% NOT compilable in this environment (verified 2026-08-02, re-verified 2026-08-05 via `tlmgr search
%% --global` and a full `texlive-full` install: `interact.cls` is genuinely not on CTAN, it is a
%% T&F-proprietary template) -- compile in an environment with the class available before
%% submitting; this pass was checked manually for balanced braces/escaping only.
%%
%% 2026-08-05 (WebSearch, NOT WebFetch-confirmed from tandfonline.com itself -- both attempts 403'd
%% again): (a) `interact.cls` IS legitimately obtainable, just not from CTAN -- Taylor & Francis
%% publishes it as an official Overleaf template gallery entry ("Taylor & Francis LaTeX template for
%% authors (Interact layout + ...)", e.g. overleaf.com/latex/templates/tagged/tandf), confirmed via
%% WebFetch to contain `interact.cls` + `interacttfpsample.tex` + `tfp.bst`; open one of those in an
%% Overleaf account to get a real compile, or download the .cls from there. (b) Two independent
%% search-engine summaries (not a primary-source fetch, since tandfonline.com itself keeps 403'ing)
%% state Advanced Robotics uses SINGLE-anonymized review (reviewers know author identity; authors
%% do not know reviewers) via ScholarOne Manuscripts, not double-anonymous -- consistent with, not
%% contradicted by, this file's own `\author{\name{Author A}\affil{...}}` block already being
%% present (unlike the double-blind T-RO/Robotica papers in this same project, which omit it). Still
%% worth a direct primary-source check before submitting, since this is corroborated-by-search, not
%% fetched from tandfonline.com itself. Abstract word limit remains genuinely unconfirmed -- search
%% results for it kept surfacing a different, similarly-named Wiley journal ("Advanced Robotics
%% Research", advanced.onlinelibrary.wiley.com) rather than this one (T&F, tandfonline.com/tadr20,
%% ISSN 0169-1864) -- do not trust a word-limit number from search without checking which journal it
%% actually describes.
...
%%   1. DONE 2026-08-03: Methodology's four architecture subsections (Baseline Pipeline,
%%      Translation-Invariant Conditioning, RHC, Sub-Segment Augmentation) written as full prose,
%%      sourced directly from cr_cfm/model.py, cr_cfm/inference.py, cr_cfm/data.py, and
%%      piper_pick_and_place.py's move_to_cr_cfm_descend() docstring -- not paraphrased from memory.
%%      Added \label{sec:architectural} to the matching Discussion subsection so the RHC subsection's
%%      forward \cref resolves. Manually re-verified: brace balance, no bare unescaped %, every \cref
%%      target has a matching \label -- still not run through an actual `interact` compile (see above).
...
The reference condition every other condition in this study is compared against: a plain,
non-learned linear interpolation in joint space from the arm's current pose to the IK-solved
descend target (\texttt{move\_to\_interpolated}), executed open-loop with zero training
investment and no learned component. This is the pipeline the wrist-orientation-selection fix
itself is layered onto in \cref{tab:critical_ablation}'s baseline+fix condition -- the fix applies
identically here as to any downstream descend-execution method, CR-CFM included.
...
CR-CFM's velocity field is conditioned on a 6-dimensional relative displacement, not an absolute
pose: at training time, $x_0^{\text{start}} - x_1^{\text{start}}$ -- the perturbed start's offset
from the clean target trajectory's own start, in joint-space radians. At inference, the true
target is exactly what is being generated and so is unavailable as a conditioning input; a proxy
is substituted instead, $x_0^{\text{start}}$ minus the training set's mean clean-start position --
``how disturbed does this start look relative to a typical clean descend.'' Conditioning on a
displacement rather than an absolute position makes the representation translation-invariant by
construction: the same relative offset conditions the model identically regardless of where in
the workspace it occurs. The train/inference substitution is a deliberate, disclosed distribution
mismatch, not an oversight -- the standard way conditional generative models handle a condition
that would otherwise require the very label being generated.

\subsection{Receding-Horizon Control (RHC) Defense Against Open-Loop Instability}
CR-CFM generates a full 16-waypoint action chunk via Euler integration of the trained velocity
field, but under receding-horizon control (RHC) only executes the first several waypoints before
re-reading the arm's actual joint position and re-generating a fresh chunk against the same fixed
target -- standard MPC-style RHC, not the open-loop full-chunk execution kept as an internal
ablation baseline. RHC was found necessary because open-loop execution commits to the full
generated motion before observing any real contact dynamics and is structurally incapable of
reactive deceleration regardless of how the training loss is weighted (confirmed directly: zeroing
the temporal-consistency regularizer's penalty in the final $30\%$ of the horizon did not reduce
terminal velocity, ruling out loss-weighting as the mechanism). Under RHC, as the current-to-target
displacement shrinks near contact, each freshly generated chunk represents proportionally smaller
motion automatically, producing deceleration without any explicit contact sensing -- though
\cref{sec:architectural} shows this closed loop still only ever re-reads the arm's own state, never
the object's position or a re-solved target.

\subsection{Sub-Segment Training Augmentation}
A training-distribution gap found by direct audit, not assumed: every original training example
had $x_0$ built from a full trajectory's own perturbed start, so the conditioning signal
(remaining distance to target) was always large (joint-space L2 norm $0.277$--$1.29$ across all
155 real training examples, never smaller) -- but RHC's later iterations present the model with
small-remaining-distance inputs as the arm approaches the target, values never seen in training.
The fix resamples additional sub-segments starting at several intermediate points along each raw
trajectory (always still ending at that trajectory's own true final waypoint), teaching the model
what small-remaining-distance conditioning should produce rather than forcing it to extrapolate.
This required no new data collection -- it purely resamples the same $n{=}155$ recordings
differently.
...
This is not a hypothetical fix. Concurrent work on reactive flow-matching control [Park and
Tulsiani, ``$\pi\mathbf{R}^2$: Reactive Real-time Flow Policies,'' arXiv:2607.26055, 2026]
diagnoses the same class of gap independently -- open-loop action-chunk execution in flow-matching
policies cannot react to fresh sensory input arriving mid-execution -- and demonstrates that
re-incorporating it (an asynchronous fast-proprioceptive/slow-vision-language conditioning split,
rather than solving the target once and never revisiting it) yields real, measured gains ($4\times$
faster closed-loop replanning, $30\%$ higher real-world success on contact-rich tasks). We do not
implement this or an equivalent object-aware re-solving mechanism for CR-CFM here -- doing so would
be a genuine architecture change, not a parameter tweak, and is out of scope for this paper's
diagnostic aims -- but its existence confirms the gap we identify is a real, fixable one, not an
artifact of how we happened to build CR-CFM specifically.
