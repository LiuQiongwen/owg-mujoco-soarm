...
- Do not edit `paper_final.tex` without checking `paper_final.pdf` diff — it is the live submission draft.
  - Target venue: RA-L. Hard limit: **8 pages** (references excluded). Currently **6 pages** (verified 2026-07-09 after adding the contact-feature diagnostic footnote to Per-Object Analysis and the cross-seed reliability paragraph to Ablation Study) — 2 pages of headroom. Re-verify page count with `latexmk -pdf paper_final.tex && pdfinfo paper_final.pdf` after any substantial edit rather than trusting this note; it has gone stale before.
