...
class Environment:
    ...
    def pick_obj_by_id(self,
        ...
        # --- robust grasp_indices handling ---
        if grasp_indices is None:
            grasp_indices = []

        # case A: already a list of grasp objects (Grasp2D/Grasp6D)
        if len(grasp_indices) > 0 and not isinstance(grasp_indices[0], (int, np.integer)):
            # treat grasp_indices as grasps directly
            ...
            if len(valid) == 0:
                # fallback: take first K grasps
                K = min(len(grasps), 4)
                valid = list(range(K))

            grasps = [grasps[i] for i in valid]

        LID = []
        for j, g in enumerate(grasps):
            x, y, z, yaw, opening_len, obj_height = g[:6]
    ...
    def place(self, target_loc: list, grasp: list, vis: bool = False):
        ...
        x, y, z, yaw, opening_len, obj_height = grasp[:6]
    ...
    def _log_ui_grasp_exec(self, mode, obj_id, grasp, success_grasp, success_target):
        ...
        """
        import os, time, json  # 确保文件顶部已经 import 过也可以去掉这行
    ...
    def draw_predicted_grasp(grasps, color=[0, 0, 1], lineIDs=[]):
        x, y, z, yaw, opening_len, obj_height = grasps[:6]
